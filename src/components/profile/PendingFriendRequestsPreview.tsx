...
const PendingFriendRequestsPreview = ({data}: FriendTemplateProp) => {
    ...
    return (
        <UserTemplateSection>
            ...
            <ButtonSection>
                <Button size="small" variant="contained" color="error" onClick={() => setOpenDeleteModal(true)}>Zrušit žádost</Button>
            </ButtonSection>
            <Dialog open={openDeleteModal} onClose={() => setOpenDeleteModal(false)}>
                <ConfirmationDialog content={`Opravdu si přejete zrušit žádost o přátelství pro uživatele ${data.name} ${data.lastName}?`} onAccept={cancelRequest} onClose={() => setOpenDeleteModal(false)} />
            </Dialog>
        </UserTemplateSection>
    );
};

export default PendingFriendRequestsPreview;
